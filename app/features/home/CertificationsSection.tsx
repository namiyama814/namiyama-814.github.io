    ...
    <section className="selection-on-dark section-frame border-b border-[#FAFAFA] bg-[#252525] text-[#FAFAFA]">
      <div className="mx-auto grid max-w-7xl grid-cols-1 sm:border-x sm:border-[#FAFAFA] lg:grid-cols-12">
        ...
        <div className="lg:col-span-9">
          ...
          <ul className="m-0 border-t border-[#FAFAFA] p-0">
            {certifications.map(({ name, year }) => (
              <li
                key={name.ja}
                className="flex items-center justify-between gap-5 border-b border-[#FAFAFA] px-5 py-6 text-sm font-bold sm:px-8 lg:px-10"
              >
                <span>{name[locale]}</span>
                <span className="text-xs font-medium tracking-[0.12em]">{year}</span>
              </li>
            ))}
          </ul>
        </div>
