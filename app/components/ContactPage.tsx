        ...
        <div className="mx-auto max-w-7xl sm:border-x sm:border-[#FAFAFA]">
          <form action="https://formspree.io/f/xgawlzep" method="POST" className="mx-auto max-w-4xl px-5 py-12 sm:px-8 lg:px-10 lg:py-20">
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
              <label className="flex flex-col gap-3 text-xs font-bold tracking-[0.08em]">{t.name}<span className="sr-only">{t.required}</span><input type="text" name="name" required autoComplete="name" placeholder={t.namePlaceholder} className="min-h-12 border-b border-[#FAFAFA] bg-transparent px-0 py-3 text-base font-medium tracking-normal text-[#FAFAFA] outline-none placeholder:text-[#FAFAFA]/60 focus-visible:border-[#FAFAFA] focus-visible:ring-2 focus-visible:ring-[#FAFAFA] focus-visible:ring-offset-4 focus-visible:ring-offset-[#252525]" /></label>
              <label className="flex flex-col gap-3 text-xs font-bold tracking-[0.08em]">{t.email}<span className="sr-only">{t.required}</span><input type="email" name="email" required autoComplete="email" placeholder={t.emailPlaceholder} className="min-h-12 border-b border-[#FAFAFA] bg-transparent px-0 py-3 text-base font-medium tracking-normal text-[#FAFAFA] outline-none placeholder:text-[#FAFAFA]/60 focus-visible:border-[#FAFAFA] focus-visible:ring-2 focus-visible:ring-[#FAFAFA] focus-visible:ring-offset-4 focus-visible:ring-offset-[#252525]" /></label>
            </div>
            <label className="mt-10 flex flex-col gap-3 text-xs font-bold tracking-[0.08em]">{t.message}<span className="sr-only">{t.required}</span><textarea name="message" required rows={7} placeholder={t.messagePlaceholder} className="min-h-40 resize-y border-b border-[#FAFAFA] bg-transparent px-0 py-3 text-base font-medium leading-7 tracking-normal text-[#FAFAFA] outline-none placeholder:text-[#FAFAFA]/60 focus-visible:border-[#FAFAFA] focus-visible:ring-2 focus-visible:ring-[#FAFAFA] focus-visible:ring-offset-4 focus-visible:ring-offset-[#252525]" /></label>
            <div className="mt-10 flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center"><p className="m-0 text-xs leading-6 text-[#FAFAFA]/75">{t.note}</p><button type="submit" className="min-h-12 border border-[#FAFAFA] px-5 py-3 text-xs font-bold tracking-[0.12em] transition-colors hover:bg-[#FAFAFA] hover:text-[#252525] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FAFAFA] focus-visible:ring-offset-4 focus-visible:ring-offset-[#252525]">{t.submit}</button></div>
          </form>
        </div>
      </section>
    </PageFrame>
